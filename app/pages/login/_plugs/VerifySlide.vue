<template>
  <div class="relative w-full">
    ...
    >
      <div
        :style="{ width: setSize.imgWidth, height: setSize.imgHeight, maxWidth: '100%' }"
    ...
    <!-- 滑块条 -->
    <div
      :style="{ width: setSize.imgWidth, height: barSize.height, lineHeight: barSize.height, maxWidth: '100%' }"
    ...
    >
      ...
      >
        ...
        <div
          ...
          @touchstart="start"
        >
          <el-icon class="verify-icon shrink-0 pointer-events-none" :size="18" :style="{ color: iconColor }">
  ...
  const move = (e) => {
    ...
    if (status.value && isEnd.value == false) {
      ...
      var blockW = parseInt(String(blockSize.value.width || barSize.value.height), 10) || 30
    ...
    if (status.value && isEnd.value == false) {
      ...
      let data = {
        ...
        pointJson: secretKey.value
          ? aesEncrypt(JSON.stringify({ x: moveLeftDistance, y: gapY.value }), secretKey.value)
      ...
      ReqCheck(data).then((res) => {
        if (res.repCode == '0000') {
          ...
          var captchaVerification = secretKey.value
            ? aesEncrypt(
                backToken.value + '---' + JSON.stringify({ x: moveLeftDistance, y: gapY.value }),
                secretKey.value
              )
            : backToken.value + '---' + JSON.stringify({ x: moveLeftDistance, y: gapY.value })
          setTimeout(() => {
